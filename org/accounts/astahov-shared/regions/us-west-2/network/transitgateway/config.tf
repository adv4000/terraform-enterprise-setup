...
terraform {
  backend "s3" {
    key          = "org/accounts/astahov-shared/regions/us-west-2/network/transitgateway/the.tfstate" # Path from 'git rev-parse --show-prefix'
...
provider "aws" {
  ...
  assume_role {
    role_arn = local.vars_account.assume_role # CrossAccount IAM Role for Terraform
  }
